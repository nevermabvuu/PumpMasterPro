...
# ============================================================================
# 4. HARDY CROSS & NETWORK SOLVER WRAPPERS
# ============================================================================
...
def solve_hardy_cross(
    ...
    """
    Hardy Cross iterative loop solver using consolidated pipe edge resistances (R, n).
    Because pseudo-nodes were eliminated, loops are minimal, well-conditioned,
    and converge without oscillation.
    """
    ...
    for it in range(max_iterations):
        ...
        for loop in loops:
            ...
            for pipe_id, sign in loop.pipe_orientations:
                ...
                # Compute consolidated resistance at current flow
                ...
                # Headloss: hf = R * |Q|^(n-1) * Q
                # Headloss oriented with loop: hf_loop = sign * hf
                hf = R * (abs(q_cur) ** (n - 1.0)) * q_cur if abs(q_cur) > 1e-12 else 0.0
                sum_h += sign * hf

                # Derivative dh/dQ: n * R * |Q|^(n-1)
            ...
            # Apply loop correction to all pipes in loop
            for pipe_id, sign in loop.pipe_orientations:
    ...
    # Calculate final consolidated results
    results = {}
